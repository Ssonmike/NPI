...
export default function PVViewer() {
    // Get params from URL if available
    const params = useParams();
    const urlWarehouseOrderId = params.warehouseOrderId;
    const urlTaskId = params.taskId;

    // Dynamic data loading state
    const [warehouseOrderId, setWarehouseOrderId] = useState(urlWarehouseOrderId || null);
    ...
    const loadDataFromUrl = async () => {
        try {
            ...
            if (urlWarehouseOrderId && urlTaskId) {
                console.log('Loading from API:', urlWarehouseOrderId, urlTaskId);

                setWarehouseOrderId(urlWarehouseOrderId);
                setTaskId(urlTaskId);

                // Fetch data from API
                const response = await fetch(`/api/warehouse-orders/${urlWarehouseOrderId}`);
                if (!response.ok) {
